...
$(document).on("vclick", "#home__page #panel-open", function() {
    $("#home__page #panel").panel("open");
});
...
$(document).on(
    "vclick",
    "#page-detail #note #frm-add-note #btn-gallery",
    function() {
...
$(document).on(
    "vclick",
    "#page-detail #note #frm-add-note #btn-camera",
    function() {
...
$(document).on("vclick", "#page-detail #btn-toggle-note", function() {
    ...
    $("#page-detail #note").css(
        "display",
        noteDisplay == "none" ? "block" : "none"
    );
});

$(document).on(
    "vclick",
    "#page-detail #note #list #btn-update-note",
    updateNoteConfirm
);
$(document).on(
    "vclick",
    "#page-detail #note #list #btn-delete-note",
    deleteNoteConfirm
);
...
function setFormInfo(form, info, isNote) {
    ...
    if (isNote) $(`${form} #note`).val(info.Note);
}
...
function setHTMLInfo(form, info, isNote, isDate = false) {
    ...
    if (isNote) $(`${form} #note`).text(info.Note);

    if (isDate) $(`${form} #date`).text(info.DateAdded);
}
    ...
    db.transaction(function(tx) {
        ...
        function transactionSuccess(tx, result) {
            ...
            db.transaction(function(tx) {
                ...
                tx.executeSql(
                    query, [info.Note, result.insertId],
                    transactionSuccess,
                    transactionError
                );
...
function addNoteImage(noteId) {
    db.transaction(function(tx) {
        ...
        for (var i = 0; i < image.length; i++) {
            var src = image[i].src;
            tx.executeSql(
                query, [src.substring(23, src.length), noteId],
                transactionSuccess,
                transactionError
            );
        }
                ...
                noteList += `
                    <div class='list' id='note-${note.Id}'>
                        <table style='white-space: nowrap;'>
                            <tr>
                                <th rowspan='2'><img src='img/logo.png' class='avatar'></th>
                                <td style='width: 100%;'><strong>RentalZ</strong></td>
    ...
    db.transaction(function(tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                addOption(
                    $("#page-detail #frm-update #type"),
                    Type,
                    "Type",
                    result.rows[0].Type
                );
                addOption(
                    $("#page-detail #frm-update #furniture"),
                    Furniture,
                    "Furniture",
                    result.rows[0].Furniture
                );
...
function displayList(list) {
    ...
    propertyList +=
        list.length == 0 ? "<li><h2>There is no property.</h2></li>" : "";

    for (let property of list) {
        propertyList += `<li><a data-details='{"Id" : ${property.Id}}'>
                <h2 style='margin-bottom: 0px;'>${property.Name}</h2>
                <p style='margin-top: 2px; margin-bottom: 10px;'><small>${
                  property.City
                }</small></p>
                
                <div>
                    <img src='img/bed.png' height='20px' style='margin-bottom: -5px;'>
                    <strong style='font-size: 13px;'>${property.Bedroom}<strong>
                    
                    &nbsp;&nbsp;
                    
                    <img src='img/home.png' height='21px' style='margin-bottom: -5px;'>
                    <strong style='font-size: 13px;'>${
                      Object.keys(Type)[property.Type]
                    }<strong>
                    
                    &nbsp;&nbsp;
                    
                    <img src='img/price-tag.png' height='20px' style='margin-bottom: -3px;'>
                    <strong style='font-size: 13px;'>${property.Price.toLocaleString(
                      "en-US"
                    )} VNĐ / month<strong>
                </div>
                <strong style="font-size: 13px;"><strong><strong style="font-size: 13px;"><strong><strong style="font-size: 13px;"><strong>
...
function getImage(saveToPhotoAlbum, sourceType) {
    ...
    function success(imageData) {
        ...
        $("#page-detail #img-preview #gallery").append(
...
function updateNote(e) {
    ...
    var id = $(this).data("details").Id;
    var message = $(
        `#page-detail #note #list #note-${id} #message-${id} #message`
    ).val();
